...
> **R9.6 三轮硬性限制**：Task 门专家讨论必须跑满 ≥ 3 轮（竞品/行业参考 → 领域+边界 → 交叉验证/安全），不足 3 轮禁止标记通过。每轮至少 2 位专家有新输入。第三轮投票才算最终有效票。
...
**自动化判定标准**：
- 🤖 自动化：严格按 `@code-kit/RULES.md` R13.0 的 7 维判定框架——任务边界清晰、技术方案已定、需求 AC 可验证、verify 可机判、无破坏性变更(R4.6)、无 schema 变更(R4.5)、无安全敏感操作。**7 维全部满足才投 🤖**，任一项不满足 → 👤
- 👤 人工：命中 R13.0 的 8 类 👤 场景之一——方案讨论/技术方案讨论/测试方案讨论/测试边界讨论/需求讨论/需求转化/用户补充/模糊边界。**任一专家投 👤 → 暂停等用户确认**，不允许多数压制少数。投 👤 必须写明具体缺失项
...
门禁通过后，**先执行 R4.7 检查点提交**，然后：`@code-kit/prompts/4-dev.md`（按波次逐个执行，`<auto>true</auto>` 的任务自动执行，`<auto>false</auto>` 的任务执行前暂停等人工确认）

> **R4.7**：Task 门通过 + TASK.md 写入后，必须 `git commit`：`docs(<id>): 3-task — TASK.md + Task门通过 (N/4)`。提交后报告 hash。
