...
- **R11.1** 网络探测优先：用环境可用的任意探测手段（如 `curl -sI` / `wget --spider`）；探测工具本身不可用时，直接按离线处理，不报错不阻塞。
...
> 代码 / 原型 / 活文档 / 决策留痕 / 执行留痕五层会各自漂移。详见 `@code-kit/prompts/S-align.md`。

- **R15.1** 进入 `7-integration` 前必须跑一次 S-align，未跑禁止集成验证。
- **R15.2** 合并动作必须经用户确认后执行；检测阶段只读。
- **R15.3** 对齐流程不修改 L1 代码——代码层漂移只能改文档/原型/决策层，或开新 CHANGE 修代码。
- **R15.4** 每轮对齐报告必须含"覆盖提交列表"与"已对齐记录"两张表，展示合并了哪些、对齐了哪些本地提交。
- **R15.5** 每轮 CHANGE 归档时顺带触发一次对齐检测。

## R16 · 调研与发现（D-discovery）

- **R16.1** D-discovery 全程零代码改动；涉代码结论只出《建议改动清单》，执行必须走新 CHANGE。
- **R16.2** 子议题拆解必须有事实依据（文件/代码/数据/用户原话），无依据不列。
- **R16.3** 人工审核门：用户明确"通过"之前，禁止写任何 discovery 产物文件。
- **R16.4** 三轮上限：实在拆不出有依据的新子议题即停止，不重复、不硬凑、不擅自开第 4 轮。
