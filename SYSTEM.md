...
- **R2.11 · 专家团讨论最低轮次（硬性限制 · R9.6）**：每个 Gate 的专家讨论**必须跑满 ≥ 3 轮**（竞品/行业分析 → 领域深化 → 交叉验证），不足 3 轮**禁止**标记 Gate 通过。第三轮投票才算最终有效票。每轮至少 2 位专家有实质性新输入。
...
- **R4.7 · 强制检查点提交（硬性限制 · 不可跳过）**：以下每个检查点**必须先 commit 再做下一件事，不允许跳过、不允许合并**。提交后向用户报告 hash。
  - 工件产出（CHANGE/REQUIREMENT/DESIGN/UI-DESIGN/TASK/TEST/REVIEW/INTEGRATION 任一写入后）
  - Gate 通过（每个门禁投票通过后）
  - Task 完成（4-dev 每个 task verify 后）
  - Wave 完成（每波次 done 后）
  - 小模块完成（独立组件/服务完成并 verify 后）
  - 阶段结束（准备进入下一阶段前）
