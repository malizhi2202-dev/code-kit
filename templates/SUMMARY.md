...
### Safety Commit（任务前 · 来自 4-dev 步骤 0）

```
✅ Safety commit: <commit-hash-short>
   - pre-T<NN> checkpoint
   - 回滚: git reset --hard <commit-hash-short>
```

### 变更规模检查（提交前 · 来自 4-dev 步骤 5.4 · R10.4）

```
✅ 变更规模检查（R10.4）：
  - 修改文件: N 个（阈值: 10）
  - 新增: +X / 删除: -Y（阈值: +500 / -100）
  - 删除文件: Z 个（阈值: 3）
  - 判定: ✅ 在阈值内
```

> 如超限，附上用户确认记录：选择 1/2/3 + 确认时间。

### Churn 检测（如触发 · R10.9）

```
- 文件 <path> 被修改 <N> 次（阈值: 5）
- 处理: 已暂停反问 / 已确认继续 / 已回滚拆分
```
