...
copycode-kit\prompts\*.md .windsurf\workflows\
```
...
> ```powershell
> mkdir -Force .windsurf\workflows
> copycode-kit\GO.md .windsurf\workflows\go.md
> copycode-kit\prompts\*.md .windsurf\workflows\
> ```
> 之后输入 `/go 设计陪诊网站` 即可。
...
| Aider | `.aider.conf.yml` 加 `read: [code-kit/SYSTEM.md]` |
...
**1-requirement**：
```
@code-kit/prompts/1-requirement.md @.specs/<id>/CHANGE.md
按 1-requirement 流程出 REQUIREMENT.md 和 .specs/CONTEXT.md。
```

**2-design**：
```
@code-kit/prompts/2-design.md @.specs/<id>/REQUIREMENT.md @.specs/CONTEXT.md
按 2-design 流程出 DESIGN.md。
```

**3-task**：
```
@code-kit/prompts/3-task.md @.specs/<id>/REQUIREMENT.md @.specs/<id>/DESIGN.md @.specs/CONTEXT.md
按 3-task 流程出 TASK.md。
```

**4-dev**（每任务一次）：
```
@code-kit/prompts/4-dev.md @.specs/<id>/TASK.md @.specs/CONTEXT.md @.specs/LESSONS.md
执行 task <T01>。
```

**5-test / 6-review / 7-integration** 类似，引用对应 prompt + 已有产物即可。

> 配合**方式 B**，可以把每条开头的 `@code-kit/prompts/...` 之前的部分省掉。
...
按顺序跑这 9 个 prompt（中间允许多轮对话）。**每个阶段出口有专家团门禁**（🛡️），全票/多数通过才进入下一阶段：

```
0-change ─🛡️G1─→ 1-requirement ─🛡️需求门─→ 2-design ─🛡️G2─→ 2a-ui-design(前端) ─🛡️G2a─→ 3-task ─🛡️Task门─→ 4-dev(逐task) ─🛡️G3─→ 5-test ─🛡️测试门─→ 6-review ─🛡️G4─→ 7-integration
```

每个阶段产出一个 `.md` 工件，存到 `./.specs/<change-id>/`。

**逐 task 自动化投票**：3-task 门禁中，专家团对每个 task 投票 🤖自动化 或 👤人工，结果写入 `<auto>` 字段，4-dev 强制执行。
...
flowchart TD
    ...
    C0 --> G1{"🛡️ <b>G1 需求方向门</b><br/>高级产品经理 + 用户评测员<br/>+ 架构师 + 安全审计师"}
    G1 -->|"✅ 通过"| C1["<b>1-requirement</b><br/>AC + 术语 + 范围切分<br/>📄 REQUIREMENT.md<br/>📄 CONTEXT.md"]
    G1 -->|"❌ 驳回"| C0
    C1 --> GR{"🛡️ <b>需求质量门</b><br/>高级产品经理 + 用户评测员<br/>+ 架构师 + 安全审计师"}
    GR -->|"✅ 通过"| C2["<b>2-design</b><br/>技术栈预选 + ADR + 风险<br/>📄 DESIGN.md"]
    GR -->|"❌ 驳回"| C1
    C2 --> G2{"🛡️ <b>G2 方案门</b><br/>架构师(M) + 研发负责人<br/>+ 领域专家 + 安全审计师"}
    G2 -->|"✅ 通过"| UI{"前端项目？"}
    G2 -->|"❌ 驳回"| C2
    UI -->|"是"| C2A["<b>2a UI-DESIGN</b><br/>调性 + design tokens<br/>📄 UI-DESIGN.md"]
    UI -->|"否"| C3
    C2A --> G2A{"🛡️ <b>G2a UI设计门</b><br/>资深UI设计师 + 用户体验官<br/>+ 前端架构师 + 无障碍专家"}
    G2A -->|"✅ 通过"| C3["<b>3-task</b><br/>原子任务 + 波次划分<br/>+ 逐task 🤖/👤 自动化投票<br/>📄 TASK.md"]
    G2A -->|"❌ 驳回"| C2A
    C3 --> GT{"🛡️ <b>Task 门</b><br/>工程效能专家 + 架构师<br/>+ 研发负责人 + 资深测试工程师"}
    GT -->|"✅ 通过"| C4["<b>4-dev 🔄</b><br/>按 <auto> 字段逐task执行<br/>📄 SUMMARY.md × N"]
    GT -->|"❌ 驳回"| C3
    C4 --> G3{"🛡️ <b>G3 代码门</b><br/>研发负责人(M) + 架构师<br/>+ 资深测试工程师 + 安全审计师"}
    G3 -->|"✅ 通过"| C5["<b>5-test</b><br/>5 轮金字塔<br/>📄 TEST.md"]
    G3 -->|"❌ 驳回"| C4
    C5 --> GTEST{"🛡️ <b>测试门</b><br/>资深测试工程师 + 研发负责人<br/>+ 高级产品经理 + 资深用户体验官"}
    GTEST -->|"✅ 通过"| C6["<b>6-review</b><br/>三轮审查 + 跨模型 spot-check<br/>📄 REVIEW.md"]
    GTEST -->|"❌ 驳回"| C4
    C6 --> G4{"🛡️ <b>G4 审查门</b><br/>资深测试工程师(M) + 架构师<br/>+ 领域专家 + 安全审计师"}
    G4 -->|"✅ 通过"| C7["<b>7-integration</b><br/>UAT + 失败诊断 + LESSONS<br/>📄 LESSONS.md"]
    G4 -->|"❌ 驳回"| C4
    ...
    style G2A fill:#ffcdd2,stroke:#b71c1c
    style GT fill:#ffcdd2,stroke:#b71c1c
    style G3 fill:#ffcdd2,stroke:#b71c1c
    style GTEST fill:#ffcdd2,stroke:#b71c1c
    style G4 fill:#ffcdd2,stroke:#b71c1c
```

**关键机制**：
- 🛡️ **8 个 Gate**（G1→需求门→G2→G2a→Task门→G3→测试门→G4），每个由 **4 名领域专家**按阶段定制（不是同一批人重复投票），全票/多数通过自动进入下一阶段，平票提交用户裁决
- 🤖 **逐 task 自动化投票**：3-task 门禁中每个 task 由研发负责人+资深测试工程师+安全审计师投票决定「自动化执行」还是「人工确认」，结果写入 `<auto>` 字段
...
每个阶段的出口都有一个**该领域专属的 4 人专家团**把守，不是同一批人反复投票。每个角色有独立的角色定义文件（`prompts/R-*.md`），定义其触发场景、边界、评估框架和输出格式。

```
G1 需求方向门 (0-change)         G2 方案门 (2-design)           G3 代码门 (4-dev)
┌──────────────────────────┐    ┌──────────────────────────┐    ┌──────────────────────────┐
│ 🟫 高级产品经理           │    │ 🟫 架构师 (Master)        │    │ 🟫 研发负责人 (Master)    │
│ 🟦 资深用户评测员         │    │ 🟦 研发负责人             │    │ 🟦 架构师                 │
│ 🟩 架构师                 │    │ 🟩 领域专家               │    │ 🟩 资深测试工程师         │
│ 🔴 安全审计师             │    │ 🔴 安全审计师             │    │ 🔴 安全审计师             │
└──────────────────────────┘    └──────────────────────────┘    └──────────────────────────┘

需求门 (1-requirement)           G2a UI设计门 (2a-ui-design)     G4 审查门 (6-review)
┌──────────────────────────┐    ┌──────────────────────────┐    ┌──────────────────────────┐
│ 🟫 高级产品经理           │    │ 🟫 资深UI设计师           │    │ 🟫 资深测试工程师 (Master) │
│ 🟦 资深用户评测员         │    │ 🟦 资深用户体验官         │    │ 🟦 架构师                 │
│ 🟩 架构师                 │    │ 🟩 前端架构师             │    │ 🟩 领域专家               │
│ 🔴 安全审计师             │    │ 🔴 无障碍专家             │    │ 🔴 安全审计师             │
└──────────────────────────┘    └──────────────────────────┘    └──────────────────────────┘

Task 门 (3-task)                 测试门 (5-test)
┌──────────────────────────┐    ┌──────────────────────────┐
│ 🟫 工程效能专家           │    │ 🟫 资深测试工程师         │
│ 🟦 架构师                 │    │ 🟦 研发负责人             │
│ 🟩 研发负责人             │    │ 🟩 高级产品经理           │
...
**裁决规则**：全票通过(4/4)→自动进入下一阶段；多数通过(3/4)→自动进入，记录反对意见；平票(2/2)→暂停提交用户裁决；全票反对→驳回。Master 角色在平票时拥有最终裁决权（R9.4）。

**角色定义文件**（12 个）：每个文件含触发场景矩阵→边界表(能做/不能做)→分步骤评估框架→检查清单→❌/✅案例→输入输出规范。见 `prompts/R-*.md` + `prompts/A-architect.md`。

### 5b. Git 最小化安全提交（R10 · 审核后自动提交）

> 不是"全部做完再交"，而是**每个 task 一个 safety commit + 门禁通过后自动前进**。这套机制让你随时可以一键回滚到上一个 safety checkpoint，不用猜"上次哪个点是好的"。

**执行流**：

```
每个 task 开始前:
  git add -A && git commit -m "safety(<change-id>): pre-<task-id> checkpoint"
  → 创建回滚点

task 执行中:
  TDD → verify → self-review → 提交

门禁通过后:
  G1 全票 → ✅ 自动进入 1-requirement
  G2 全票 → ✅ 自动进入 3-task（或 2a）
  Task门 全票 → ✅ 自动进入 4-dev · 🤖task 直接开始执行
  G3 全票 → ✅ 自动进入 5-test
  测试门 全票 → ✅ 自动进入 6-review
  G4 全票 → ✅ 自动进入 7-integration
...
**关键规则**（R10）：
- 变更规模 > 10 文件 / > 500 行 / > 100 删除行 / ≥ 3 删除文件 → 暂停反问用户
- 连续失败 ≥ 3 次 → 自动 safety commit → 反问用户
- 门禁通过后**禁止**再问"要继续吗？"（R13.4），直接自动执行

### 5c. Task 自动化执行决策（`<auto>` 字段 · R13）

> 不是所有 task 都该自动跑。涉及破坏性变更、schema 变更、安全敏感操作的 task 必须**先经专家团投票**，标记为 👤 人工确认后才执行。
...
│                                                          │
│  🗳️ T04: 👤 人工 (2/3 多数)                               │
│     🟦 研发负责人: 👤 — 涉及 schema 变更，需确认迁移策略   │
│     🟩 资深测试工程师: 🤖 — verify 可自动判定              │
│     🔴 安全审计师: 👤 — 数据迁移需人工审查                │
│                                                          │
│  结果写入 TASK.md:                                        │
│  <task id="T01" ...><auto>true</auto></task>             │
│  <task id="T04" ...><auto>false</auto></task>            │
└──────────────────────────────────────────────────────────┘
                         │
                         ▼
┌─ 4-dev 阶段 ────────────────────────────────────────────┐
...
│  步骤 0 · 自动化门禁检查（强制）：                        │
│                                                          │
│  <auto>true</auto>  → 🤖 直接执行，不暂停                │
│  <auto>false</auto> → 👤 暂停，输出：                    │
│    "专家团判定 T04 需人工确认后执行。                     │
│     理由：涉及 schema 变更，需确认迁移策略                │
│     1. 执行  2. 跳过  3. 回 3-task 重评"                │
│                                                          │
│  缺 <auto> 字段 → 停止，要求回 3-task 补跑门禁           │
└──────────────────────────────────────────────────────────┘
```
...
| 🤖 自动化 | 👤 人工确认 |
|---|---|
| task 边界清晰、verify 可机判 | 涉及破坏性变更（R4.6） |
| 无 schema 变更 | 涉及 schema 变更（R4.5） |
| 无安全敏感操作 | 安全/权限改动 |
| 非 UI 视觉验证 | verify 需要人眼判断 |
| 不涉及第三方 API 联调 | 涉及生产数据/真实支付 |
...
> 每个角色文件定义：触发场景（哪些门禁）→ 边界（能做/不能做）→ 分步骤评估框架 → 检查清单（含 ❌/✅ 案例）→ 输入输出规范。

| 文件 | 角色 | 出现于 |
|---|---|---|
| `prompts/A-architect.md` | 架构师 | G1/需求门/G2/Task/G3/G4 |
| `prompts/R-高级产品经理.md` | 高级产品经理 | G1/需求门/测试门 |
| `prompts/R-资深用户评测员.md` | 资深用户评测员 | G1/需求门 |
| `prompts/R-安全审计师.md` | 安全审计师（合并安全架构师） | G1/需求门/G2/G3/G4/Task |
| `prompts/R-研发负责人.md` | 研发负责人（合并资深研发工程师） | G2/Task/G3/测试门 |
| `prompts/R-领域专家.md` | 领域专家 | G2/G4 |
| `prompts/R-资深UI设计师.md` | 资深UI设计师 | G2a |
| `prompts/R-资深用户体验官.md` | 资深用户体验官 | G2a/测试门 |
| `prompts/R-前端架构师.md` | 前端架构师 | G2a/G2 |
| `prompts/R-无障碍专家.md` | 无障碍专家 | G2a |
...
>code-kit 的设计是 **token 多花、单窗压力小、产物有外溢价值**。下表帮你按场景选挡位。
> 完整估算逻辑见 `@code-kit/GO.md` 的「Token 预算估计」段。
...
>code-kit 的两个高频担忧：**AI 不按既有架构开发** + **AI 乱删乱改不相关代码**。下表是每条护栏拦的具体事故。
...
| 项目 | 首次 | 之后每个 change |
|---|---|---|
| 5 万行老项目首次跑code-kit | +15-30k（intel-scan）= ~265-560k | ~250-530k（与 greenfield 同） |
...
4. **8 Gate 专家团门禁**：每个阶段出口由 4 名该领域专家对抗审核，全票/多数通过自动进入下一阶段。角色按阶段定制（不是同一批人重复投票）
5. **逐 task 自动化投票**：每个 task 由专家团投票决定 🤖自动化执行或 👤需人工确认，结果写入 `<auto>` 字段，4-dev 强制执行
6. **审查至少三轮**：spec 合规 + 代码质量(6维) + UI 视觉(前端)；建议第四轮跨模型 spot-check
7. **失败是输入而不是终点**：UAT 失败自动产 fix-plan，回到 `4-dev`，最多 3 轮
8. **artifact-first，不是 phase-gated**：阶段可压缩、可回炉，但 `.md` 不能缺席。它不是 phase-gated，而是 artifact-gated
