...
**集成能力**：10 阶段主流程（0-change→…→7-integration）+ 8 道专家团门禁（12 个领域角色）+ 逐 task 自动化投票 + 5 道老项目安全护栏 + 横向命令（入场扫描/架构演进/健康巡检/一键换皮）+ 11 个独立可用的工件模板 + 4 个参考文档。纯 markdown，无运行时依赖。
...
**新增策略**（相比原始版）：8 Gate 专家团门禁（每阶段领域专属角色）、逐 task 自动化投票（`<auto>` 字段）、角色定义文件体系（`R-*.md`）、5 道 brownfield 护栏、MVP 模式、极简模式、token 预算估算。
