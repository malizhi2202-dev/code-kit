...
> **目的**：验证 4-dev / 5-test / 6-review 的「装了 brooks-lint」路径 A 与「内置回退」路径 B 都能产出 **4 要素 + R/T 编号** 的统一格式，让code-kit 下游（fix 任务追加、TASK.md 编号延续）无缝接收。
>
...
**只要两条路径都过「格式一致性」+「下游可接收」段，融合就成功了**。「路径差异」段的 gap 是 brooks-lint 本身的价值（带书本引用 + 系统化覆盖），不是code-kit 集成的问题。
