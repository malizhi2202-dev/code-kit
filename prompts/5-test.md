...
> **R9.6 三轮硬性限制**：测试门专家讨论必须跑满 ≥ 3 轮（竞品/行业参考 → 领域+边界 → 交叉验证/安全），不足 3 轮禁止标记通过。每轮至少 2 位专家有新输入。第三轮投票才算最终有效票。
...
> **R4.7**：测试门通过 + TEST.md 写入后，必须 `git commit`：`docs(<id>): 5-test — TEST.md + 测试门通过 (N/4)`。提交后报告 hash。
