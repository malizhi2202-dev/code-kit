...
> **R9.6 硬性限制**：每个 Gate 的专家讨论**必须跑满至少 3 轮**（竞品分析→领域深化→交叉验证），不足 3 轮禁止标记通过。详见 `@code-kit/RULES.md` R9.6。
...
> **逐 task 自动化投票**：3-task 门禁中，每个 task 由研发负责人 + 资深测试工程师 + 安全审计师投票决定「🤖 自动化」还是「👤 人工介入」。**投票必须严格按 `@code-kit/RULES.md` R13.0 的 7 维判定框架**执行——任一专家投 👤（命中方案讨论/技术方案讨论/测试方案讨论/测试边界讨论/需求讨论/需求转化/用户补充/模糊边界 8 类场景之一）即暂停等用户补充，不允许多数压制少数。结果写入 `<auto>` 字段，4-dev 步骤 0 强制执行。
...
## 第四步半 · 检查点提交 + 运行时埋点（强制 · 每个阶段结束时执行）

### 4.5-A · 检查点提交（R4.7 硬性限制 · 先于埋点）

每个阶段/工件/Gate/Task/Wave 完成后，**必须先提交再继续**。**不允许跳过、不允许静默、不允许等用户提醒。**

提交信息格式与完整检查点清单见 `@code-kit/RULES.md` R4.7。

AI 在路由声明的「第一动作」一栏末尾追加 `R4.7: <检查点> → commit`，并在每次提交后向用户报告：
```
✅ R4.7 提交：<commit-hash> — <提交信息>
```
