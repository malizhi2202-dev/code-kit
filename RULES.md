...
- **R9.1** 4 Gate 强制：G1(需求), G2(方案), G3(代码), G4(测试)，每 Gate ≥ 4 角色。
- **R9.2** 交叉对抗：每个 🔴 必须被另一角色二次确认，记录完整对抗过程。
- **R9.3** 自动化裁决：全票通过→自动进入下阶段；多数(≥75%)→采纳多数；僵局(平票)→提交用户。**禁止**在全票/多数情况下问"是否继续"。
- **R9.4** Master 角色：G1/G2→架构师, G3→研发负责人, G4→测试专家。
