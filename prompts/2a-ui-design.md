...
> **R9.6 三轮硬性限制**：G2a 专家讨论必须跑满 ≥ 3 轮（竞品/行业参考 → 领域+边界 → 交叉验证/无障碍），不足 3 轮禁止标记通过。每轮至少 2 位专家有新输入。第三轮投票才算最终有效票。
...
门禁通过后，**先执行 R4.7 检查点提交**，然后：`@code-kit/prompts/3-task.md` —— 拆任务时把 design tokens 物化到代码（CSS variables / theme 文件）作为第一批任务

> **R4.7**：G2a 门禁通过 + UI-DESIGN.md 写入后，必须 `git commit`：`docs(<id>): 2a-ui-design — UI-DESIGN.md + G2a 通过 (N/4)`。提交后报告 hash。
